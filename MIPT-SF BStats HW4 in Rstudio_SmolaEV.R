...
# Нарисовать три графика распределения размера опухоли в одной системе координат

ggplot(tumor, aes(x = therapy, y = value)) +
  geom_boxplot() +
  labs(x = "Группа лечения", y = "Размер Опухоли") +
  ggtitle("Размер Опухоли в Группах лечения")

# Провести тест ANOVA на датасете
anova_result <- aov(value ~ therapy, data = tumor)

# Вывести результаты теста ANOVA 
summary(anova_result)

# По результатам теста параметр p-value (Pr (>F) в таблице) выше, чем уровень значимости 0,05
# Соответственно, нет статистически достоверных доказательств того, что между тремя терапевтическими группами есть разница


# Провести попарные сравнения групп с использование критерия Тьюки 
tukey_result <- TukeyHSD(anova_result)

# Вывести результаты попарных сравнений
print(tukey_result)

# Результаты сравнений показывают разницу в значениях размеров опухолей между группами
# Однако, для всех попарных сравнений величина P выше уровня значимости 0,05
# Соответсвенно, разница в размерах опухоли между группами не имеет статистической значимости
# Это подтверждает предварительные выводы по результатам теста anova, указанными выше.
